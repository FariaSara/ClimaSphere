...
// Minimal Leaflet loader (no react-leaflet dependency)
function useLeaflet() {
...
export default function Bushfire() {
  ...
  const [trend, setTrend] = useState({ temp: [], wind: [], humidity: [], dryness: [] });
    ...
    fetch(url)
      ...
      .then((gj) => {
        const features = (gj.features || []).filter((f) =>
          STATE_LIST.includes((f.properties.STATE_NAME || f.properties.NAME || '').trim())
        );
  ...
  useEffect(() => {
    if (!L || !geojson) return;
    if (!mapRef.current) {
      mapRef.current = L.map('bushfire-map', {
        center: [-25.0, 134.0],
        zoom: 4,
        minZoom: 3,
        maxZoom: 10,
    ...
    const style = (feature) => {
      ...
      return { color, weight: selected.state === name ? 3 : 1.5, fillOpacity: risk ? 0.25 : 0.1 };
    };
    const layer = L.geoJSON(geojson, {
      ...
      onEachFeature: (feature, l) => {
        ...
        l.on('click', (e) => {
          ...
          setSelected({ state: name, lat: parseFloat(lat.toFixed(4)), lon: parseFloat(lng.toFixed(4)) });
          setRiskByState((prev) => ({ ...prev }));
          l.bindPopup(`${name}<br/>Lat: ${lat.toFixed(4)}, Lon: ${lng.toFixed(4)}`).openPopup(e.latlng);
        });
      },
      interactive: true,
    }).addTo(map);
    ...
    // Point-in-polygon helpers for clicks outside polygons
    ...
    const onMapClick = (e) => {
      ...
      if (!inside) {
        setError('Invalid location: Please select within Australia’s 8 states/territories.');
      }
    };
  ...
  const callPredict = async () => {
    ...
    try {
      const baseUrls = [
        `http://127.0.0.1:8000/predict/bushfire/all?date=${encodeURIComponent(date)}`,
        `http://localhost:8000/predict/bushfire/all?date=${encodeURIComponent(date)}`,
        `/predict/bushfire/all?date=${encodeURIComponent(date)}`,
      ];
      const fetchWithTimeout = async (url, method = 'POST', ms = 15000) => {
      ...
      for (const url of baseUrls) {
        try {
          res = await fetchWithTimeout(url, 'POST');
      ...
      if (!data || !Array.isArray(data)) {
        throw new Error(`Prediction service unavailable. Ensure backend is running on http://127.0.0.1:8000.\nDetails:\n${errors.join('\n')}`);
      }
      const mapping = {};
      ...
      // Build 7-day trends from the returned single-day values (synthetic variations for UI)
      const t0 = Number(selectedRow?.temperature ?? 0) || 0;
      const w0 = Number(selectedRow?.wind_speed ?? 0) || 0;
      const h0 = Number(selectedRow?.humidity ?? 0) || 0;
      const d0 = Number(selectedRow?.vegetation_dryness ?? 0) || 0;
      const temp = Array.from({ length: 7 }, (_, i) => ({ d: i, v: Number((t0 + Math.sin(i / 2) * 1.5).toFixed(1)) }));
      const wind = Array.from({ length: 7 }, (_, i) => ({ d: i, v: Number((w0 + Math.cos(i / 3) * 1.5).toFixed(1)) }));
      const humidity = Array.from({ length: 7 }, (_, i) => ({ d: i, v: Math.max(0, Math.min(100, Number((h0 + Math.sin(i / 3) * 3).toFixed(1)))) }));
      const dryness = Array.from({ length: 7 }, (_, i) => ({ d: i, v: Math.max(0, Math.min(100, Number((d0 + Math.cos(i / 2) * 2).toFixed(1)))) }));
      setTrend({ temp, wind, humidity, dryness });
    } catch (e) {
  ...
  const callEarlyPredict = async () => {
    ...
    try {
      const baseUrls = [
        `http://127.0.0.1:8000/predict/bushfire/early?date=${encodeURIComponent(earlyDate)}`,
        `http://localhost:8000/predict/bushfire/early?date=${encodeURIComponent(earlyDate)}`,
        `/predict/bushfire/early?date=${encodeURIComponent(earlyDate)}`,
      ];
  ...
  return (
    <div className="min-h-screen bg-dark-bg">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="p-6">
        <button onClick={() => navigate('/')} className="text-cyan-glow hover:text-white transition-colors duration-300 flex items-center space-x-2">
          <span>←</span>
      ...
      {/* Main Content */}
      <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="container mx-auto px-6 py-8">
        <div className="text-center">
          <motion.h1 initial={{ scale: 0.95 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 100 }} className="text-5xl font-bold text-white mb-3 glow-text">
            Bushfire Prediction 🔥
          </motion.h1>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div id="bushfire-map" className="w-full h-[520px] rounded-xl overflow-hidden border border-cyan-glow/30 bg-dark-secondary/40" />
          </div>
          <div>
            ...
            <div className="mt-4 p-5 rounded-xl bg-dark-secondary/50 border border-yellow-400/30">
              ...
              {early && (
                <div className="mt-3 text-sm text-gray-200 space-y-1">
                  ...
                  <div>Forecast on {earlyDate}: <span className="px-2 py-0.5 rounded text-black" style={{ background: RISK_COLOR[early.risk_level] }}>{early.risk_level}</span> <span className="text-gray-400">({early.bushfire_probability}% prob)</span></div>
        ...
        {result && (
          <div className="mt-8 space-y-6">
            ...
            <div className={`p-4 rounded-xl border ${result.risk_level === 'High' ? 'border-red-400/40 bg-red-400/10' : result.risk_level === 'Medium' ? 'border-yellow-400/40 bg-yellow-400/10' : 'border-green-400/40 bg-green-400/10'}`}>
              <div className="text-white font-semibold">{result.risk_level} Bushfire Risk – {result.state}</div>
              <div className="text-gray-200 text-sm mt-1">{result.ai_advice}</div>
            </div>

            {/* Conditionally show metrics only when all core fields are available */}
            {(() => {
              const hasMetrics = (
                typeof result?.temperature === 'number' &&
                typeof result?.wind_speed === 'number' &&
                typeof result?.humidity === 'number' &&
                typeof result?.vegetation_dryness === 'number'
              );
              ...
              return (
                <>
                    ...
                    <table className="min-w-full text-left">
                      <thead className="bg-dark-secondary/70 text-gray-300">
                        <tr>
                          ...
                          <th className="px-4 py-2">Vegetation Dryness (%)</th>
                      ...
                      <tbody>
                        <tr className="border-t border-cyan-glow/10">
                          ...
                          <td className="px-4 py-2 text-gray-200">{result.temperature}</td>
                          <td className="px-4 py-2 text-gray-200">{result.wind_speed}</td>
                          <td className="px-4 py-2 text-gray-200">{result.humidity}</td>
                          <td className="px-4 py-2 text-gray-200">{result.vegetation_dryness}</td>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <StatCard title="Temperature" value={`${result.temperature}`} subtitle="°C" />
                    <StatCard title="Wind Speed" value={`${result.wind_speed}`} subtitle="m/s" />
                    <StatCard title="Humidity" value={`${result.humidity}`} subtitle="%" />
                    <StatCard title="Vegetation Dryness" value={`${result.vegetation_dryness}`} subtitle="%" />
                    <StatCard title="Risk Score" value={`${result.risk_score ?? 0}`} />
                  </div>
                </>
              );
            })()}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Chart title="Temperature (7-day)" data={trend.temp} color="#f97316" />
              <Chart title="Wind Speed (7-day)" data={trend.wind} color="#60a5fa" />
              <Chart title="Humidity (7-day)" data={trend.humidity} color="#22c55e" />
              <Chart title="Vegetation Dryness (7-day)" data={trend.dryness} color="#eab308" />
            </div>
          </div>
        )}
      </motion.div>
