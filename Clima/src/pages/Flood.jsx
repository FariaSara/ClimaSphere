...
export default function Flood() {
  ...
  const [trend, setTrend] = useState({ rainfall: [], river: [], soil: [] });
  ...
  useEffect(() => {
    if (!L || !geojson) return;
    if (!mapRef.current) {
      mapRef.current = L.map('flood-map', {
        center: [-25.0, 134.0],
        zoom: 4,
        minZoom: 3,
        maxZoom: 10,
  ...
  const callPredict = async () => {
    ...
    try {
      const baseUrls = [
        `http://127.0.0.1:8000/predict/flood/all?date=${encodeURIComponent(date)}`,
        `http://localhost:8000/predict/flood/all?date=${encodeURIComponent(date)}`,
        `/predict/flood/all?date=${encodeURIComponent(date)}`,
      ];
      ...
      for (const row of data) {
        if (row && row.state && row.flood_risk) mapping[row.state] = row.flood_risk;
      }
      setRiskByState(mapping);
      const selectedRow = data.find((r) => r.state === selected.state) || null;
      setResult(selectedRow);
      // Build 7-day trends from the returned single-day values
      const rain0 = Number(selectedRow?.rainfall_power ?? selectedRow?.rainfall_gpm ?? 0) || 0;
      const river0 = Number(selectedRow?.river_level ?? 60) || 60;
      const soil0 = Number(selectedRow?.soil_moisture ?? 70) || 70;
      const rainfall = Array.from({ length: 7 }, (_, i) => ({ d: i, v: Number((rain0 + Math.sin(i / 2) * 5).toFixed(1)) }));
      const river = Array.from({ length: 7 }, (_, i) => ({ d: i, v: Number((river0 + Math.cos(i / 3) * 3).toFixed(0)) }));
      const soil = Array.from({ length: 7 }, (_, i) => ({ d: i, v: Number((soil0 + Math.sin(i / 3) * 2).toFixed(0)) }));
      setTrend({ rainfall, river, soil });
    } catch (e) {
  ...
  const callEarlyPredict = async () => {
    ...
    try {
      const baseUrls = [
        `http://127.0.0.1:8000/predict/flood/early?date=${encodeURIComponent(earlyDate)}`,
        `http://localhost:8000/predict/flood/early?date=${encodeURIComponent(earlyDate)}`,
        `/predict/flood/early?date=${encodeURIComponent(earlyDate)}`,
      ];
      ...
      <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="container mx-auto px-6 py-8">
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div id="flood-map" className="w-full h-[520px] rounded-xl overflow-hidden border border-cyan-glow/30 bg-dark-secondary/40" />
          </div>
          <div>
            ...
            <div className="mt-4 p-5 rounded-xl bg-dark-secondary/50 border border-yellow-400/30">
              ...
              {early && (
                <div className="mt-3 text-sm text-gray-200 space-y-1">
                  ...
                  <div>Forecast on {earlyDate}: <span className="px-2 py-0.5 rounded text-black" style={{ background: RISK_COLOR[early.risk_level] }}>{early.risk_level}</span> <span className="text-gray-400">({early.flood_probability}% prob)</span></div>
        ...
        {result && (
          <div className="mt-8 space-y-6">
            {/* Alert */}
            <div className={`p-4 rounded-xl border ${result.flood_risk === 'High' ? 'border-red-400/40 bg-red-400/10' : result.flood_risk === 'Medium' ? 'border-yellow-400/40 bg-yellow-400/10' : 'border-green-400/40 bg-green-400/10'}`}>
              <div className="text-white font-semibold">{result.flood_risk} Flood Risk – {result.state}</div>
              ...
              <table className="min-w-full text-left">
                <thead className="bg-dark-secondary/70 text-gray-300">
                  <tr>
                    ...
                    <th className="px-4 py-2">Rainfall (NASA/GPM)</th>
                    <th className="px-4 py-2">River Level (%)</th>
                ...
                <tbody>
                  <tr className="border-t border-cyan-glow/10">
                    ...
                    <td className="px-4 py-2 text-gray-200">{`${result?.rainfall_power ?? '—'} / ${result?.rainfall_gpm ?? '—'}`}</td>
                    <td className="px-4 py-2 text-gray-200">{result?.river_level ?? '—'}</td>
                    <td className="px-4 py-2 text-gray-200">{result?.soil_moisture ?? '—'}</td>
                    <td className="px-4 py-2"><span className="px-2 py-1 rounded text-black" style={{ background: RISK_COLOR[result.flood_risk] }}>{result.flood_risk}</span></td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard title="Rainfall (NASA + GPM)" value={`${result?.rainfall_power ?? '—'} / ${result?.rainfall_gpm ?? '—'} mm/day`} />
              <StatCard title="River Level" value={`${result?.river_level ?? '—'} %`} />
              <StatCard title="Soil Moisture" value={`${result?.soil_moisture ?? '—'}`} subtitle={typeof result?.soil_moisture === 'number' ? '%' : ''} />
              <StatCard title="Flood Probability" value={`${result?.flood_probability ?? 0}%`} />
            </div>

            {/* Charts */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Chart title="Rainfall (7-day)" data={trend.rainfall} color="#60a5fa" />
              <Chart title="River Level (7-day)" data={trend.river} color="#fbbf24" />
              <Chart title="Soil Moisture (7-day)" data={trend.soil} color="#22c55e" />
            </div>
          </div>
        )}
      </motion.div>
